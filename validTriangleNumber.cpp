...
class Solution {
    ...
    int triangleNumber(vector<int>& nums) {
        int count = 0, i = 0, j = 0, k = 0, size = nums.size();
        std::sort(nums.begin(), nums.end());
        for (; i < size - 2; i++) {
            k = i + 2;
            for (j = i + 1; j < size - 1 && nums[i] != 0; j++) {
...
void    testResult(vector<int> tmp) {
    Solution sol;
    int start = clock();
    cout << "Num of he valid combinations = " << sol.triangleNumber(tmp) << endl;
    int end = clock();
    cout << "T: " << ((float)(end) - (float)start) / CLOCKS_PER_SEC << endl;
}

void    testListVector(vector<int> tmp) {
    cout << "Vector of ";
